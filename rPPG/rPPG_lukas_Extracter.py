...
class rPPG_Lukas_Extracter():
    ...
    def calc_ppg(self,frame):
        try:
            ...
            if self.rPPG:
             self.rPPG.append(self.rPPG[-1])
            else:
             self.rPPG.append([0, 0, 0])
        
        
    def track_Local_motion_lukas(self):        
     if len(self.cropped_gray_frames) < 2:
        return

     if self.cropped_gray_frames[-2].shape != self.cropped_gray_frames[-1].shape:
        # Reset points if frames mismatch
        self.points = np.zeros((1,1,2), dtype=np.float32)
        self.points[0,0,0] = 380
        self.points[0,0,1] = 280
        return

     self.points, st, err = cv2.calcOpticalFlowPyrLK(
        self.cropped_gray_frames[-2], self.cropped_gray_frames[-1], self.points, None, **self.lk_params)
